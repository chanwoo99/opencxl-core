...
def create_vcs_and_rp() -> Tuple[CxlVirtualSwitch, List[CxlPortDevice], CxlRootPortDevice]:
    ...
    dsp_devices = [
        DownstreamPort(transport_connection=CxlConnection(), port_index=1),
        DownstreamPort(transport_connection=CxlConnection(), port_index=2),
        DownstreamPort(transport_connection=CxlConnection(), port_index=3),
    ]
...
async def test_virtual_switch_manager_test_bind_and_unbind():
    ...
    for port_index in range(1, vppb_counts + 1):
        ...
        dsp = DownstreamPort(transport_connection=connection, port_index=port_index)
