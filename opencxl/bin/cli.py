...
def start(
    ...
    config_components = ["switch", "ld-group", "host-group", "complex-host-group"]
    for c in comp:
    ...
    if "ld" in comp:
        ...
        t_ld.start()
    if "ld-group" in comp:
        t_group = threading.Thread(target=start_ld_group, args=(ctx, config_file))
        threads.append(t_group)
        t_group.start()
