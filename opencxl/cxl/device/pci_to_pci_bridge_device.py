...
class DownRouting(RunnableComponent):
    def __init__(
        self,
...
class UpRouting(RunnableComponent):
    def __init__(
        self,
        downsteam_connection: CxlConnection,
        ...
        self._sources = [self._dsc.cfg_fifo.target_to_host,self._dsc.mmio_fifo.target_to_host,self._dsc.cxl_mem_fifo.target_to_host,self._dsc.cxl_cache_fifo.target_to_host]
...
class PPBDevice(RunnableComponent):
    def __init__(
        ...
        self._up_routing = UpRouting(self._downstream_connection,self._upstream_connections, self._is_mld)
        self._down_routings =[DownRouting(self._downstream_connection,upstream, idx, self._is_mld) for (idx,upstream) in enumerate(self._upstream_connections)]
    ...
    async def _run(self):
        ...
        run_tasks = [
            create_task(self._up_routing.run())
        ]
        for task in self._down_routings:
            run_tasks.append(create_task(task.run()))

        wait_tasks = [
            create_task(self._up_routing.wait_for_ready())
        ]
        for task in self._down_routings:
